...
local function pick()
  ...
  -- Track fenced code blocks so a `#` comment inside one isn't mistaken for a
  -- heading. Toggle on any line that opens/closes a ``` or ~~~ fence (the
  -- fence marker may be indented and may carry an info string like ```lua).
  ...
  for lnum, line in ipairs(lines) do
    ...
    elseif not in_fence then
      ...
      if hashes and text then
        ...
        table.insert(
          headings,
          { level = level, text = text, path = path, lnum = lnum }
        )
  ...
  Snacks.picker.pick({
    ...
    format = function(item)
      ...
      -- Display the bare heading (item.heading); the indentation conveys
      -- hierarchy. item.text holds the full ancestor path, but that's only
      -- the fuzzy-match ordinal — showing it here would render the parent
      -- prefix ("H2 Parent / Title") that we don't want.
      --
      -- Match positions are computed against item.text (the path), so they
      -- don't map onto the heading display; we intentionally skip the
      -- position highlighting rather than mis-highlight the wrong columns.
      return {
        { "H" .. item.level .. " ", level_hl },
        { indent, "SnacksPickerFile" },
        { item.heading, "SnacksPickerFile" },
      }
    end,
